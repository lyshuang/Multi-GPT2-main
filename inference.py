...
def main():
    ...
    if args.model_type == 'gpt2':
        if args.single_input:
            model = GPT2DoubleHeadsModel.from_pretrained('/data/lys/MultiGPTData/gpt2-small')
        else:
            model = GPT2EncoderDecoderModel.from_pretrained('/data/lys/MultiGPTData/gpt2-small')
        tokenizer = GPT2Tokenizer.from_pretrained('/data/lys/MultiGPTData/gpt2-small')
    elif args.model_type == 'gpt':
        model = OpenAIGPTEncoderDecoderModel.from_pretrained('/data/lys/MultiGPTData/openai-gpt')
        tokenizer = OpenAIGPTTokenizer.from_pretrained('/data/lys/MultiGPTData/openai-gpt')
    ...
    trainer = Trainer(model,
                      ...
                      full_input=trainer_config.full_input,
                      uncertainty_loss=args.uncertainty_loss,
                      best_model_path="/home/lys/workspace/Multi-GPT2-main/test",
    ...
    def external_metrics_func(full_references, full_predictions, epoch, metric=None):
        ...
        rouge_l, f1_score, avg_length = nlp_metrics(references_file_path, predictions_file_path, root_path=save_path)
