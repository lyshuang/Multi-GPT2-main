...
class FacebookDataset(Dataset):
        ...
        with open(path, 'r', encoding='utf-8') as file:
            ...
            for line in file.readlines():
                ...
                if dialog_line[0].startswith('your persona:'):
                    persona_info = dialog_line[0].replace('your persona: ', '') # 去掉'your persona: '
                    if persona_info[-1] == '.' and persona_info[-2] != ' ': #persona行句末的'.'替换成' .'，这一步操作现在还不知道有啥作用
            ...
            for chat in tqdm(data):
                ...
                if only_final:
                    ...
                    for i, replica in enumerate(chat['dialog'], 1):
                        ...
                        if not i % 2:# 将persona、dialog、candidates对应词向量存储起来，数据集一行变对应一个元组[persona_info, dialog, candidates]
                            if chat['candidates']:
    ...
    def __init__(self, paths, vocab, *, max_lengths=512,  max_y_length=80, min_infos=2, dialog_embeddings=False,
        ...
        if isinstance(paths, str):
            paths = [paths] #'datasets/ConvAI2/dummy.txt'

        self.augment = augment # False
        self.aug_syn_proba = aug_syn_proba # 0
        self.aug_vary_length = aug_vary_length # True

        self.vocab = vocab # gpt2的tokenizer
        self.max_lengths = max_lengths # 511
        self.max_y_length = max_y_length # 80
        self.min_infos = min_infos # 2
        self.dialog_embeddings = dialog_embeddings # True
        self.use_start_end = use_start_end
        self.negative_samples = negative_samples  # -1 => include all candidates in data instance
        self.max_history_size = max_history_size # -1
        self.single_input = single_input # False
        self.data_type = data_type # 'persona'

        # self.data = self._parse_data(paths, vocab, data_type, parsed_data)

        if cache and os.path.exists(cache):
            self.data = torch.load(cache) # 'datasets/dummy_cache_gpt2'
    ...
    # 给data中每个index下的persona_info、dialog_history、query加上相应的标识符
    # 返回加了标识符的persona_info、dialog_history、query[0]、query[1:]
    def __getitem__(self, idx):
        persona_info, dialog, candidates = self.data[idx]

        if len(persona_info):
            persona_info = self._augment(persona_info, info=True)
            persona_info = sum(persona_info, []) #将persona_info的维度由原先的二维变为1维
            #如果是单输入，则persona_info则仅仅加上vocab的起始标志info_bos_id
            #否则需要对persona_info的长度进行限制
            #   1.不需要dialog_embedding,则persona_info取(最长(max_lengths)-2)+info_bos_id+info_eos_id(use_start_end=true),
            #      Q_lys:max_lengths-2还不知道为什么
            #   2.需要dialog_embedding,则personal_info的每个元素都需要加上info_dialog_id，Q_lys：这种做法是否存在着资源浪费
        ...
        # self.vocab.info_bos_id=self.vocab.info_dialog_id=50260
        # persona_info：
        # [1820, 50260]
        # ...[13, 50260]
        ...
        sentences = []
        # dialog[-1:]:dialog的最后一行,query都应该加上vocab.talker1_bos_id和vocab_eos_id
        for y in (dialog[-1:] + candidates):
            if self.single_input:
                ...
                if self.dialog_embeddings:
                    y = [[tok, self.vocab.sent_dialog_id] for tok in y] #给dialog的每个标量都加上dialog_id
                sentences.append(y)
